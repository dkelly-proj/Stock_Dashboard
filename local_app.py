...
# Setup table
symbols = static_data.static_symbols #td_helper.get_symbols()
df_quotes = pd.read_json(static_data.static_quotes) #td_helper.get_radar_quotes(symbols)

# Setup tweets table
try:
    df_tweets = pd.read_json(static_data.static_tweets) #td_helper.fetch_tweets(tweet_symbols)
except:
...
df_cs_one = pd.read_json(static_data.static_candle) #df_p_hist.query("Symbol == @volatile_tickers[0]").reset_index(drop=True)
